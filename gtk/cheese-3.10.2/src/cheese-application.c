/* cheese-application.c generated by valac 0.22.0.43-7c3715, the Vala compiler
...
static void cheese_application_real_startup (GApplication* base) {
	...
	if (!_tmp2_) {
		g_error ("cheese-application.vala:85: Unable to initialize libcheese-gtk");
	}
	G_APPLICATION_CLASS (cheese_application_parent_class)->startup ((GApplication*) G_TYPE_CHECK_INSTANCE_CAST (self, GTK_TYPE_APPLICATION, GtkApplication));
...
static void cheese_application_on_camera_state_flags_changed (CheeseApplication* self, GstState new_state) {
	...
	switch (_tmp6_) {
		...
		case GST_STATE_NULL:
		{
			...
			cheese_main_window_camera_state_change_null (_tmp22_);
			break;
		}
		default:
		{
...
GType cheese_application_get_type (void) {
	...
	if (g_once_init_enter (&cheese_application_type_id__volatile)) {
		...
		cheese_application_type_id = g_type_register_static (GTK_TYPE_APPLICATION, "CheeseApplication", &g_define_type_info, 0);
